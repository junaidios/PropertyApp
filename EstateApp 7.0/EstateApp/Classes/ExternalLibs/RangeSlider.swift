...
//      fatalError("init(coder:) has not been implemented")
  ...
  func updateLayerFrames(){
    ...
    trackLayer.frame = bounds.rectByInsetting(dx: 0.0, dy: 15.15)
    
    trackLayer.setNeedsDisplay()
    
    let lowerThumbCenter = CGFloat(positionForValue(lowerValue))
    
    lowerThumbLayer.frame = CGRectMake(lowerThumbCenter - thumbWidth/2, 0.0, thumbWidth, thumbWidth)
    lowerThumbLayer.setNeedsDisplay()
    
    let upperThumbCenter = CGFloat(positionForValue(upperValue))
    
    upperThumbLayer.frame = CGRectMake(upperThumbCenter - thumbWidth/2, 0.0, thumbWidth, thumbWidth)
    upperThumbLayer.setNeedsDisplay()
